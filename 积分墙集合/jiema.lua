    ...
    return {
	    login=(function() 
			...
			for i=1,5,1 do
				toast("获取token\n"..i.."次共5次")
                mSleep(1500)
				local lx_url = 'http://43.249.193.217:81/logon/username='..User..'&password='..Pass
				log(lx_url)
                RetStr = get_lx(lx_url)
				if RetStr then
					RetStr = strSplit(RetStr,"|")
		...
		getPhone=(function()
            local RetStr=""
			local url___ = "http://43.249.193.217:81/Getnumber/id="..PID.."&token="..token
			log(url___)
			RetStr = get_lx(url___)
			if RetStr ~= "" and  RetStr ~= nil then
				log(RetStr)
			...
			if RetStr[1] == 1 or RetStr[1]== '1' then
				...
				for k,v in ipairs(blackPhone) do
					if phone_title == v then
						local lx_url =	'http://43.249.193.217:81/Addblacklist/id='..PID..'&phone='..number..'&token='..token
						log(lx_url)
						get_lx(lx_url);
						log("拉黑->"..number)
						return false
	    ...
	    getMessage=(function()
            ...
            for i=1,25,1 do
				mSleep(3000)
				RetStr = get_lx("http://43.249.193.217:81/Getsms/id="..PID.."&token="..token.."&phone="..number)
				log(RetStr);
				if RetStr then
					local arr = strSplit(RetStr,"|") 
					if arr[1] == '1' then 
